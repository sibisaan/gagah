...
# Fungsi untuk menangani pesan email dan menyimpan email dalam user_data
async def handle_email(update: Update, context: CallbackContext):
    ...
    context.user_data['state'] = 'password'  # Mengubah state menjadi password
    await update.message.reply_text("Please enter your password:")

# Fungsi untuk menangani password setelah email dimasukkan
async def handle_password(update: Update, context: CallbackContext):
    ...
    # Mengecek state yang ada di user_data
    if context.user_data.get('state') == 'password':
        if not user_email:
            # Jika email tidak ditemukan dalam user_data, minta kembali email
        ...
        # Jika state bukan password, maka menunjukkan bahwa ada kesalahan dalam alur
        await update.message.reply_text("Unexpected state. Please start over.")
