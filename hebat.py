...
async def login_user(update: Update, context: CallbackContext):
    ...
    if len(email_password) != 2:
        await update.message.reply_text("Format tidak benar. Silakan kirimkan email dan password Anda.")
        return LOGIN
    ...
    if login_response:
        ...
        await update.message.reply_text(f"Login berhasil! Anda telah berhasil login sebagai {email}. Sekarang, silakan kirimkan email lama Anda.")
        return EMAIL_OLD
    else:
        await update.message.reply_text("Login gagal. Periksa kembali email dan password Anda.")
        return LOGIN
...
async def receive_old_email(update: Update, context: CallbackContext):
    ...
    if send_verification_code(old_email):
        await update.message.reply_text(f"Kode verifikasi telah dikirim ke {old_email}. Sekarang, silakan kirimkan email baru Anda.")
        return EMAIL_NEW
    else:
        await update.message.reply_text("Gagal mengirimkan kode verifikasi. Coba lagi nanti.")
...
async def receive_new_email(update: Update, context: CallbackContext):
    ...
    await update.message.reply_text(f"Email baru Anda adalah: {new_email}. Sekarang, silakan masukkan kode verifikasi yang Anda terima.")
    return CODE_VERIFICATION_NEW

# Fungsi menerima kode verifikasi email baru
async def receive_verification_code_new(update: Update, context: CallbackContext):
    ...
    if confirm_change_email(old_email, new_email, verification_code):
        await update.message.reply_text(f"Email berhasil diubah dari {old_email} ke {new_email}.")
    else:
        await update.message.reply_text("Gagal mengubah email. Periksa kembali kode verifikasi yang dimasukkan.")
    
    return ConversationHandler.END

# Fungsi cancel percakapan
async def cancel(update: Update, context: CallbackContext):
    await update.message.reply_text("Proses perubahan email dibatalkan.")
    return ConversationHandler.END

# Menjalankan bot
def main():
    # Ganti 'YOUR_BOT_TOKEN' dengan token bot Telegram yang didapatkan dari BotFather
