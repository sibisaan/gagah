...
async def handle_email(update: Update, context: CallbackContext):
    ...
    context.user_data['state'] = 'waiting_for_password'  # Mengubah state menjadi 'waiting_for_password'
    await update.message.reply_text("Please enter your password:")

# Fungsi untuk menangani password dan login
async def handle_password(update: Update, context: CallbackContext):
    # Mengecek apakah user sudah mengirimkan email dan sekarang menunggu password
    if context.user_data.get('state') != 'waiting_for_password':
    ...
    if user_token:
        ...
        if login_response:
            # Kirim seluruh respon JSON dari server
            await update.message.reply_text(f"Login successful! Server response: {login_response}")
        else:
            await update.message.reply_text("Login failed.")
        
        # Setelah password dimasukkan dan proses login selesai, reset state agar tidak meminta password lagi
        context.user_data['state'] = 'done'  # Mengubah state menjadi 'done' setelah login selesai
    else:
...
def main():
    ...
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_email))  # Email first
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_password))  # Password after email
